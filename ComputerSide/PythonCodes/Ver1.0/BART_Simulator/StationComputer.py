...
#==============================================================================================================           
    def getWCSD2(self,train):        
        ...
        a = -4
        d3 = train.getVelocity()*self.tjp + (1/2)*self.ap*self.tjp*self.tjp + (1/6)*self.jp*self.tjp*self.tjp*self.tjp + (1/2)*a*self.tjp*self.tjp
        v3 = train.getVelocity() + self.ap*self.tjp + (1/2)*self.jp*self.tjp*self.tjp + a*self.tjp
        d4 = v3*self.mc + (1/2)*a*self.mc*self.mc
        v4 = v3 + a*self.mc
        d5 = v4*self.tjb + (1/6)*self.jb*self.qfsmc*self.tjb*self.tjb*self.tjb + (1/2)*a*self.tjb*self.tjb
        v5 = v4 + (1/2)*self.jb*self.qfsmc*self.tjb*self.tjb + a*self.tjb
        d6 = v5*self.t6 + (1/2)*self.bfs*self.t6*self.t6 + (1/2)*a*self.t6*self.t6
...
# Updating new commanded velocity and acceleration:                
           ...
           # print "\t\t\tacm: " + str(commandedAcc) + " vcm: " + str(commandedSpeed) + " NextStop is: "+nextObj.getName()+" in:" + str(nextStopDistance-trainPosition)+" meters" + " WCSD2: " + str(wcsd2) +"\n"
            train.update()
